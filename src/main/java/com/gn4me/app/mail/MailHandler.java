...
public boolean sendActiviteAccountMail(User receiver, String path, Transition transition) throws IOException {
		...
		try {
			...
			logger.debug("[" + transition.getId()
					+ "] Going to build Basic Mail Info from User: "+ receiver + " resetPath: " + path);
			...
			// Set Mail Subject and Title 
			basicInfo.setSubject(appInfo.getName() + " | "+ActiviteAccount.ACTIVITE_ACCOUNT.getValue(transition.getLanguage()));
			basicInfo.setTitle(ActiviteAccount.ACTIVITE_ACCOUNT.getValue(transition.getLanguage()));
			
			// Set Mail Body
			basicInfo.getElements().add(ActiviteAccount.START.getValue(transition.getLanguage()));

			basicInfo.getElements().add(ActiviteAccount.BODY.getValue(transition.getLanguage()));
			// Set Mail Actions
			action = new MailAction();
			action.setLable(ActiviteAccount.ACTIVITE_ACCOUNT.getValue(transition.getLanguage()));
			action.setLink(path);
			basicInfo.getActions().add(action);
			
			// Set Mail Notes.
			basicInfo.setNote(ActiviteAccount.NOTE.getValue(transition.getLanguage()));
